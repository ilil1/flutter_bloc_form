...
class Model<T> extends BaseModel {
  ...
  static T? _dataFromJson<T>(dynamic data) {
    ...
    return switch (T) {
      const (ExceptionModel) => ExceptionModel.fromJson(data),
      const (bool) || const (int) || const (String) => data,
      _ => null,
    } as T;
  }
  ...
  static Map<String, dynamic>? _dataToJson<T>(T? data) {
    return switch (T) {
      const (ExceptionModel) => (data as ExceptionModel).toJson(),
      const (bool) || const (int) || const (String) => <String, dynamic>{'data': data},
      _ => null,
    };
  }
